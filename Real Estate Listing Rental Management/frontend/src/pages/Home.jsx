      ...
      {selectedType ? (
        <>
          ...
          {txFilter === '' ? (
            <>
              <section className="section">
                <div className="row" style={{ justifyContent: 'space-between' }}>
                  <h2>{types.find(t=>t.key===selectedType)?.label}: For Rent</h2>
              ...
              <section className="section">
                <div className="row" style={{ justifyContent: 'space-between' }}>
                  <h2>{types.find(t=>t.key===selectedType)?.label}: For Sale</h2>
                  <span className="muted">Top 10 results</span>
                </div>
                {loading && <div className="muted" style={{ marginTop: 12 }}>Loading listings...</div>}
                {error && !loading && <div className="muted" style={{ marginTop: 12 }}>{error}</div>}
            ...
            <section className="section">
              <div className="row" style={{ justifyContent: 'space-between' }}>
                <h2>{types.find(t=>t.key===selectedType)?.label}: {txFilter==='rental' ? 'For Rent' : 'For Sale'}</h2>
              </div>
              {loading && <div className="muted" style={{ marginTop: 12 }}>Loading listings...</div>}
              {error && !loading && <div className="muted" style={{ marginTop: 12 }}>{error}</div>}
              {!loading && !error && (
